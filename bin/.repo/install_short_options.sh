...
################### >>>> FUNCTIONS <<<<<< ###################
usage(){
    echo "usage:  $0  -f FC_PLAT [-p PREFIX_DIR] [-o OPT_LIB] [-c -w -d -h]"
    echo "   -f : specifies the actual platform/compiler to use [intel,intel_debug,gnu,gnu_debug]."
    echo "   -p : specifies the target directory [default=FC_PLAT]."
    echo "   -o : installs a single optional library [arpack blas fftpack lapack minpack quadpack dmftt]."
    echo "   -c : wipes out all the compilations." 
    echo "   -w : compiles the DMFT_TOOLS library within SciFor."
    echo "   -d : turns on the debug compilation flag."
    echo "   -h : this help."
    exit
}
...
OPT=
WRKDIR=$(pwd)
WRK_INSTALL=$WRKDIR/_install ; if [ ! -d $WRK_INSTALL ];then echo "$0: can not find _install directory";exit 1;fi
...
while getopts "hdf:p:o:cw" FLAG;do
    case $FLAG in
	(d) DEBUG=0 ;;
	(h) usage; exit 1 ;;
	(f) PLAT=$OPTARG
	    [[ ! $LIST_FC =~ (^|[[:space:]])"$PLAT"($|[[:space:]]) ]] && {
		echo "Incorrect PLAT: $PLAT";
		exit 1
	    } ;;
	(p) PREFIX=$OPTARG ;;
	(o) OPT=$OPTARG
	     [[ ! "$LIST_OPT" =~ (^|[[:space:]])"$OPT"($|[[:space:]]) ]] && {
		 echo "Incorrect OPT: $OPT";
		 exit 1
	     } ;;
	(c) OPT=clean ;;
	(w) OPT=wdmftt ;;
...
#check that the mandatory option -f is present:
[[ $LIST_ARGS =~ "-f" ]] || usage
...
#INSTALL SINGLE OPTIONAL LIB IF REQUIRED:
case $OPT in
    arpack)
	if [ $WMPI == 1 ];then
