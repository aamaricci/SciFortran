...
contains
  ...
  subroutine fftgf_rw2rt(func_in,func_out,M)
    ...
    complex(8),dimension(2*M)              :: fIN,fOUT
    complex(8),dimension(-M:M)             :: dummy_out
    complex(8),dimension(-M:M),intent(out) :: func_out
    integer*8                             :: plan
    N=2*M ; forall(i=1:2*M)fIN(i) = func_in(i-M-1)
    call dfftw_plan_dft_1d(plan,N,fIN,fOUT,FFTW_BACKWARD,FFTW_ESTIMATE)
    call dfftw_execute_dft(plan,fIN,fOUT)
    call dfftw_destroy_plan(plan)
    !Manipulations of the out-array:
    forall(i=1:n)func_out(i-M-1)=fOUT(i)    ![1,2*M=N]---> [-M,M-1]
    forall(i=-M:-1)dummy_out(i+M)=func_out(i)!g[0,M-1]<--- x[-M,-1]
    forall(i=0:M-1)dummy_out(i-M)=func_out(i)!g[-L,-1]<--- x[0,L-1]
  ...
  subroutine fftgf_rt2rw(func_in,func_out,M)
    ...
    integer(8)                            :: plan
    N=2*M ; forall(i=1:2*M)fIN(i) = func_in(i-M-1)
    call dfftw_plan_dft_1d(plan,N,fIN,fOUT,FFTW_FORWARD,FFTW_ESTIMATE)
    call dfftw_execute_dft(plan,fIN,fOUT)
    call dfftw_destroy_plan(plan)
    ex=-1.d0
    do i=1,n
       ex=-ex
       func_out(i)=ex*fOUT(i)
    enddo
  ...
  subroutine fft_iw2tau(gw,gt,beta,L)
    ...
    complex(8),dimension(2*L)  :: tmpGw,tmpGt2
    ...
    do i=1,L
       ...
       tmpGw(2*i-1)= zero
    enddo
    call fftgf_rw2rt(tmpGw,tmpGt2,2*L) ;tmpGt2=2.d0*tmpGt2/beta
    forall(i=1:2*L)tmpGt(i-L-1)=2.d0*dble(tmpGt2(i))/beta
  ...
  subroutine fft_tau2iw(gt,gw,beta,L)
    ...
    call fftgf_rt2rw(xgw,gw,2*L) ;gw=2.d0*gw/beta
    !call four1(xgw,2*L,1)
    !Exit:
    forall(i=1:2*L)gw(i)=xgw(i)*beta/dble(L)/2.d0
