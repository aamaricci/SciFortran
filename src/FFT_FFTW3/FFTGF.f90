...
contains
  ...
  subroutine fftgf_rw2rt(func_in,func_out,M)
    ...
    integer*8                              :: plan
    fIN = func_in
    call dfftw_plan_dft_1d(plan,2*M,fIN,fOUT,FFTW_BACKWARD,FFTW_ESTIMATE)
    call dfftw_execute_dft(plan,fIN,fOUT)
    call dfftw_destroy_plan(plan)
    forall(i=1:2*M)func_out(i-M-1)=fOUT(i)    ![1,2*M=N]---> [-M,M-1]
  ...
  subroutine fftgf_rt2rw(func_in,func_out,M)
    ...
    complex(8),dimension(2*M)             :: fIN,fOUT
    integer(8)                            :: plan
    forall(i=1:2*M)fIN(i) = func_in(i-M-1)
    call dfftw_plan_dft_1d(plan,2*M,fIN,fOUT,FFTW_FORWARD,FFTW_ESTIMATE)
    call dfftw_execute_dft(plan,fIN,fOUT)
    call dfftw_destroy_plan(plan)
    ex=-1.d0
    do i=1,2*M
