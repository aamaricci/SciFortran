...
!###############################################################
module IPT_FUNX_KELDYSH
  use IPT_VARS_GLOBAL
  private

  integer,save                 :: loop=1 
  type(keldysh_equilibrium_gf) :: fg,fg0,sigma
  type(matsubara_gf)           :: gf,gf0,sf
...
contains
  ...
  !COMMENT  : 
  !+-------------------------------------------------------------------+
  function solve_ipt_keldysh(fg0_,label_,iprint_) result(sigma_)
    ...
    if(loop==1)then
       ...
       if(.not.gf%status)call allocate_gf(gf,L)
       if(.not.gf0%status)call allocate_gf(gf0,L)
       if(.not.sf%status)call allocate_gf(sf,L)
  ...
  subroutine simpurity
    ...
    ! if(trim(PTmethod)=="spt")then
    !    fg%ret%w = one/(one/fg0%ret%w - sigma%ret%w)
    !    call update_G0_keldysh(fg%ret%w)
    !    do i=-L,L
    !       sigma%less%t(i)=(U**2)*(fg0%less%t(i)**2)*fg0%gtr%t(-i)
    !       sigma%gtr%t(i) =(U**2)*(fg0%gtr%t(i)**2)*fg0%less%t(-i)
    !    enddo
