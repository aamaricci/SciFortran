...
!
  ...
  integer ( kind = 4 ) k
  integer ( kind = 4 ) na
  real ( kind = 8 ), parameter :: taui =  0.866025403784439d0
  real ( kind = 8 ), parameter :: taur = -0.5d0
  real ( kind = 8 ) ti2
  real ( kind = 8 ) tr2
...
!
  ...
  real ( kind = 8 ) ti4
  real ( kind = 8 ) ti5
  real ( kind = 8 ), parameter :: ti11 =  0.9510565162951536d0
  real ( kind = 8 ), parameter :: ti12 =  0.5877852522924731d0
  real ( kind = 8 ) tr2
  real ( kind = 8 ) tr3
  real ( kind = 8 ) tr4
  real ( kind = 8 ) tr5
  real ( kind = 8 ), parameter :: tr11 =  0.3090169943749474d0
  real ( kind = 8 ), parameter :: tr12 = -0.8090169943749474d0
...
!
  ...
  else if (lensav < 2*n + int(log( real ( n, kind = 8 ) )/log( 2.0d0 )) + 4) then
    ier = 2
...
!    2, input parameter LENSAV not big enough.
  ...
  if (lensav < 2*n + int(log( real ( n, kind = 8 ) )/log( 2.0d0 )) + 4) then
    ier = 2
...
!
  ...
  real ( kind = 8 ) wsave(lensav)
      ...
      if ( ldim < l ) then
        ...
        return
      else if (lensav < 2*l + int(log( real ( l, kind = 8 ))/log( 2.0d0 )) + &
                          2*m + int(log( real ( m, kind = 8 ))/log( 2.0d0 )) +8) then
        ier = 2
...
!
      iw = 2*l+int(log( real ( l, kind = 8 ) )/log( 2.0d0 )) + 3
      call cfftmb(l, 1, m, ldim, c, (l-1) + ldim*(m-1) +1, &
           wsave(iw), 2*m + int(log( real ( m, kind = 8 ))/log( 2.0d0 )) + 4, &
           work, 2*l*m, ier1)
...
!
      ...
      call cfftmb (m, ldim, l, 1, c, (m-1)*ldim + l, &
           wsave(iw), 2*l + int(log( real ( l, kind = 8 ) )/log( 2.0d0 )) + 4, &
           work, 2*m*l, ier1)
      if (ier1 /= 0) then
        ier = 20
        call xerfft ('cfft2b',-5)
...
!
  ...
  if ( lensav < 2 * l + int ( log ( real ( l, kind = 8 ) ) &
    / log ( 2.0d0 ) ) + 2 * m + int ( log ( real ( m, kind = 8 ) ) &
    / log ( 2.0d0 ) ) + 8 ) then
    ier = 2
  ...
  call cfftmi ( m, & 
       wsave(2*l+int(log( real ( l, kind = 8 ) )/log( 2.0d0 )) + 3), &
       2*m + int(log( real ( m, kind = 8) )/log( 2.0d0 )) + 4, ier1)
...
!
  ...
  real ( kind = 8 ) x(inc,*)
      ...
      if (n-2) 102,101,103
  101 ssqrt2 = 1.0d0 / sqrt ( 2.0d0 )
      tsqx = ssqrt2*x(1,2)
      x(1,2) = 0.5d0 *x(1,1)-tsqx
      x(1,1) = 0.5d0 *x(1,1)+tsqx
  102 return
...
!
  ...
  real ( kind = 8 ) wsave(lensav)
      ...
      do 101 k=1,n
         ...
         wsave(k) = cos(fk*dt)
  101 continue
      lnsv = n + int(log( real ( n, kind = 8 ) )/log( 2.0d0 )) +4
...
!
  ...
  101 ssqrt2 = 1.0d0 / sqrt ( 2.0d0 )
      ...
      do m=1,lj,jump
        tsqx = ssqrt2*x(m,2)
        x(m,2) = 0.5d0 * x(m,1)-tsqx
        x(m,1) = 0.5d0 * x(m,1)+tsqx
      end do
...
!
  ...
  real ( kind = 8 ) wsave(lensav)
      ...
      lnsv = nm1 + int(log( real ( nm1, kind = 8 ) )/log( 2.0d0 )) +4
...
!
  ...
  124 lenx = inc*(nm1-1)  + 1
      ...
      fnm1s2 = real ( nm1, kind = 4 ) / 2.0d0 
      dsum = 0.5D+00 * dsum
      x(1,1) = fnm1s2*x(1,1)
      if(mod(nm1,2) /= 0) go to 30
      x(1,nm1) = x(1,nm1)+x(1,nm1)
   30 fnm1s4 = real ( nm1, kind = 4 ) / 4.0d0
...
!
  ...
  real ( kind = 8 ) xi
      ...
      if (n-2) 200,101,102
  101 x1h = x(1,1)+x(1,2)
      x(1,2) = 0.5d0 * (x(1,1)-x(1,2))
      x(1,1) = 0.5d0 * x1h
      go to 200
  102 if ( 3 < n ) go to 103
      x1p3 = x(1,1)+x(1,3)
      tx2 = x(1,2)+x(1,2)
      x(1,2) = 0.5d0 * (x(1,1)-x(1,3))
      x(1,1) = 0.25d0 *(x1p3+tx2)
      x(1,3) = 0.25d0 *(x1p3-tx2)
      go to 200
...
!
 ...
 124  continue
      ...
      lnsv = nm1 + int(log( real ( nm1, kind = 4 ))/log( 2.0d0 )) + 4
      lnwk = lot*nm1
...
!
  ...
  102 if ( 3 < n ) go to 103
      do 112 m=1,lj,jump
         x1p3 = x(m,1)+x(m,3)
         tx2 = x(m,2)+x(m,2)
         x(m,2) = 0.5d0 * (x(m,1)-x(m,3))
         x(m,1) = 0.25d0 * (x1p3+tx2)
         x(m,3) = 0.25d0 * (x1p3-tx2)
  112 continue
      go to 200
  103 m1 = 0
      do 113 m=1,lj,jump
...
!
  ...
  real ( kind = 8 ) wa2(ido)
      ...
      m1d = (m-1)*im1+1
      m2s = 1-im2
      arg= 2.0d0 * 4.0d0 * atan ( 1.0d0 ) / 3.0d0
      taur=cos(arg)
      taui=sin(arg)
      do 101 k=1,l1
          m2 = m2s
          do 1001 m1=1,m1d,im1
          m2 = m2+im2
         ch(m2,1,k,1) = cc(m1,1,1,k)+ 2.0d0 *cc(m1,ido,2,k)
         ch(m2,1,k,2) = cc(m1,1,1,k)+( 2.0d0 *taur)*cc(m1,ido,2,k) &
         -( 2.0d0 *taui)*cc(m1,1,3,k)
         ch(m2,1,k,3) = cc(m1,1,1,k)+( 2.0d0 *taur)*cc(m1,ido,2,k) &
         + 2.0d0 *taui*cc(m1,1,3,k)
 1001     continue
  101 continue
      if (ido == 1) return
...
!
  ...
  real ( kind = 8 ) wa4(ido)
      ...
      m1d = (m-1)*im1+1
      m2s = 1-im2
      arg= 2.0d0 * 4.0d0 * atan ( 1.0d0 ) / 5.0d0 
      tr11=cos(arg)
      ti11=sin(arg)
      tr12=cos( 2.0d0 *arg)
      ti12=sin( 2.0d0 *arg)
      ...
      do 1001 m1=1,m1d,im1
         m2 = m2+im2
         ch(m2,1,k,1) = cc(m1,1,1,k)+ 2.0d0 *cc(m1,ido,2,k)+ 2.0d0 *cc(m1,ido,4,k)
         ch(m2,1,k,2) = (cc(m1,1,1,k)+tr11* 2.0d0 *cc(m1,ido,2,k) &
         +tr12* 2.0d0 *cc(m1,ido,4,k))-(ti11* 2.0d0 *cc(m1,1,3,k) &
         +ti12* 2.0d0 *cc(m1,1,5,k))
         ch(m2,1,k,3) = (cc(m1,1,1,k)+tr12* 2.0d0 *cc(m1,ido,2,k) &
         +tr11* 2.0d0 *cc(m1,ido,4,k))-(ti12* 2.0d0 *cc(m1,1,3,k) &
         -ti11* 2.0d0 *cc(m1,1,5,k))
         ch(m2,1,k,4) = (cc(m1,1,1,k)+tr12* 2.0d0 *cc(m1,ido,2,k) &
         +tr11* 2.0d0 *cc(m1,ido,4,k))+(ti12* 2.0d0 *cc(m1,1,3,k) &
         -ti11* 2.0d0 *cc(m1,1,5,k))
         ch(m2,1,k,5) = (cc(m1,1,1,k)+tr11* 2.0d0 *cc(m1,ido,2,k) &
         +tr12* 2.0d0 *cc(m1,ido,4,k))+(ti11* 2.0d0 *cc(m1,1,3,k) &
         +ti12* 2.0d0 *cc(m1,1,5,k))
 1001          continue
  101 continue
...
!
  ...
  110    l2 = l1
  111 continue
      sn = 1.0d0 / real ( n, kind = 4 )
      tsn =  2.0d0 / real ( n, kind = 4 )
      tsnm = -tsn
      modn = mod(n,2)
      nl = n-2
      if(modn /= 0) nl = n-1
...
!
 ...
 102  srt3s2 = sqrt( 3.0d0 )/ 2.0d0 
  ...
  104 continue
      ...
      do 123 m=1,lj,jump
         m1 = m1+1
         xh(m1,ns2+2) =  4.0d0 * x(m,ns2+1)
  123 continue
  124 do 127 m=1,lot
         xh(m,1) = 0.0d0
  127 continue 
      lnxh = lot-1 + lot*(np1-1) + 1
      lnsv = np1 + int(log( real ( np1, kind = 4 ))/log( 2.0d0 )) + 4
      lnwk = lot*np1
...
!
 ...
 102  ssqrt3 = 1.0d0 / sqrt ( 3.0d0 )
  ...
  127 continue 
      ...
      xh(m,np1) = xh(m,np1)+xh(m,np1)
   20 continue
   30 sfnp1 = 1.0d0 / real ( np1, kind = 4 )
      m1 = 0
      do 125 m=1,lj,jump
         m1 = m1+1
         x(m,1) = 0.5d0 * xh(m1,1)
  ...
  125 continue
      do 105 i=3,n,2
         ...
         do 115 m=1,lj,jump
            m1 = m1+1
            x(m,i-1) = 0.5d0 * xh(m1,i)
            dsum(m1) = dsum(m1)+ 0.5d0 * xh(m1,i-1)
            x(m,i) = dsum(m1)
  115    continue
  105 continue
      if (modn /= 0) go to 200
...
!
  ...
  do k = 1, l1
    ch(1,1,k,1) = cc(1,1,1,k) + 2.0d0 * cc(1,ido,2,k)
    ch(1,1,k,2) = cc(1,1,1,k) + ( 2.0d0 * taur ) * cc(1,ido,2,k) &
      - ( 2.0d0 *taui)*cc(1,1,3,k)
    ch(1,1,k,3) = cc(1,1,1,k) + ( 2.0d0 *taur)*cc(1,ido,2,k) &
      + 2.0d0 *taui*cc(1,1,3,k)
  end do
...
!
  ...
  real ( kind = 8 ) wa4(ido)
      ...
      do k=1,l1
         ch(1,1,k,1) = cc(1,1,1,k)+ 2.0d0 *cc(1,ido,2,k)+ 2.0d0 *cc(1,ido,4,k)
         ch(1,1,k,2) = (cc(1,1,1,k)+tr11* 2.0d0 *cc(1,ido,2,k) &
         +tr12* 2.0d0 *cc(1,ido,4,k))-(ti11* 2.0d0 *cc(1,1,3,k) &
         +ti12* 2.0d0 *cc(1,1,5,k))
         ch(1,1,k,3) = (cc(1,1,1,k)+tr12* 2.0d0 *cc(1,ido,2,k) &
         +tr11* 2.0d0 *cc(1,ido,4,k))-(ti12* 2.0d0 *cc(1,1,3,k) &
         -ti11* 2.0d0 *cc(1,1,5,k))
         ch(1,1,k,4) = (cc(1,1,1,k)+tr12* 2.0d0 *cc(1,ido,2,k) &
         +tr11* 2.0d0 *cc(1,ido,4,k))+(ti12* 2.0d0 *cc(1,1,3,k) &
         -ti11* 2.0d0 *cc(1,1,5,k))
         ch(1,1,k,5) = (cc(1,1,1,k)+tr11* 2.0d0 *cc(1,ido,2,k) &
         +tr12* 2.0d0 *cc(1,ido,4,k))+(ti11* 2.0d0 *cc(1,1,3,k) &
         +ti12* 2.0d0 *cc(1,1,5,k))
      end do
...
!
  ...
  else if (lensav < n + int(log( real ( n, kind = 8 ) )/log( 2.0d0 )) +4) then
    ier = 2
...
!
  ...
  real ( kind = 8 ) wsave(lensav)
      ...
      if (lensav < n + int(log( real ( n, kind = 8 ) )/log( 2.0d0 )) +4) then
        ier = 2
...
!  verify lensav
!
      lwsav =   l+int(log( real ( l, kind = 8 ) )/log( 2.0d0 ))+4
      mwsav =   2*m+int(log( real ( m, kind = 8 ) )/log( 2.0d0 ))+4
      mmsav =   m+int(log( real ( m, kind = 8 ) )/log( 2.0d0 ))+4
      modl = mod(l,2)
      modm = mod(m,2)
...
!
      ...
      call rfftmb(m,ldim,l,1,r,m*ldim,wsave(1), &
           l+int(log( real ( l, kind = 8 ) )/log( 2.0d0 ))+4,work,lenwrk,ier1)
...
!
  ...
  if (lenx < inc*(n-1) + 1) then
    ...
    return
  else if (lensav < n/2 + n + int(log( real ( n, kind = 8 ) )/log( 2.0d0 )) +4) then
    ier = 2
...
!
  ...
  real ( kind = 8 ) wsave(lensav)
      ...
      if (lensav < n/2 + n + int(log( real ( n, kind = 8 ) )/log( 2.0d0 )) +4) then
        ier = 2
...
!
 ...
 30   fnp1s4 = real ( np1, kind = 4 ) / 4.0d0
         x(1,1) = fnp1s4*xh(1)
         dsum = x(1,1)
      do i=3,n,2
            x(1,i-1) = fnp1s4*xh(i)
...
subroutine sintmi ( n, wsave, lensav, ier )
  ...
  lnsv = np1 + int ( log ( real ( np1, kind = 8 ) )/ log ( 2.0d0 ) ) + 4
